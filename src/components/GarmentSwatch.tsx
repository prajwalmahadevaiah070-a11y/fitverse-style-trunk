...
function shapeFor(p: Product): Shape {
  ...
  if (s.includes("lehenga") || s.includes("anarkali") || s.includes("dress") || s.includes("bridal"))
    return "dress";
  if (s.includes("denim") || s.includes("pant")) return "pants";
  if (s.includes("jacket") || s.includes("blazer") || s.includes("sherwani") || s.includes("bandh"))
    return "jacket";
  return "top";
}

const PATHS: Record<Shape, string> = {
  saree: "M50 8 C34 14 30 30 34 48 C38 70 30 84 24 96 L76 96 C74 78 72 56 70 40 C68 22 64 12 50 8 Z",
...
export function GarmentSwatch({
  ...
  return (
    ...
    >
      ...
      <path d={PATHS[shape]} fill={`url(#${gid})`} />
      <path
        d={PATHS[shape]}
        fill="none"
